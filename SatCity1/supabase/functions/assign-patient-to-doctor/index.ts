// supabase/functions/assign-patient-to-doctor/index.ts
//
// TRIGGER: Database Webhook on INSERT into lab_results table
//
// LOGIC:
//  1. Read diagnosed_condition + patient_id from new lab_result row
//  2. Find all doctors whose specialty[] array contains a match
//  3. Filter those by availability_status = 'available'
//  4. If available specialists exist:
//       → pick the one with fewest active patients (load balancing)
//       → insert patient_doctor_assignments row
//       → update patient record
//       → notify the chosen doctor (realtime broadcast)
//  5. If NO available specialist:
//       → broadcast 'emergency_available_patient' to ALL matching doctors
//       → the first to accept will trigger accept-emergency-patient
//
