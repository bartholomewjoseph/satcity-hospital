...
import { BackgroundGrid, AnimatedText, TiltCard, MagneticButton, DotBackground } from "../components/aceternity";
    ...
    <nav className="fixed top-4 left-1/2 -translate-x-1/2 z-50">
      <div className={`flex items-center gap-1 px-2 py-2 rounded-full border backdrop-blur-md transition-all ${scrolled ? "bg-white/90 shadow-lg border-neutral-200" : "bg-white/70 border-white/50"}`}>
        ...
        <div className="hidden md:flex items-center gap-1 text-sm text-neutral-700">
          <a href="#features" className="px-3 py-1.5 rounded-full hover:bg-neutral-100">Features</a>
          <a href="#roles" className="px-3 py-1.5 rounded-full hover:bg-neutral-100">Roles</a>
          <a href="#modules" className="px-3 py-1.5 rounded-full hover:bg-neutral-100">Modules</a>
        </div>
        <Link to="/emergency">
...
export function LandingPage() {
  return (
    <div className="min-h-screen bg-white">
      ...
      <BackgroundGrid className="pt-32 pb-24">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-neutral-200 bg-white text-xs text-neutral-600 mb-6">
              <span className="h-1.5 w-1.5 rounded-full bg-emerald-500 animate-pulse" />
              Realtime hospital operations — powered by Supabase
            </div>
          ...
          <div className="mt-20 grid grid-cols-2 md:grid-cols-4 gap-4">
            {[
              ...
              { label: "Specialist Doctors", value: "86" },
              { label: "Lab Results Today", value: "342" },
              { label: "Avg. Response Time", value: "< 3min" },
            ].map((s) => (
      ...
      <section id="features" className="py-20 bg-neutral-50 border-y border-neutral-200">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-14">
            <h2 className="text-4xl font-bold text-neutral-900"><AnimatedText text="Everything your hospital needs" /></h2>
      ...
      <section id="roles" className="py-20">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-14">
            ...
            <h2 className="mt-4 text-4xl font-bold text-neutral-900">Tailored for every team member</h2>
            <p className="mt-3 text-neutral-600 max-w-2xl mx-auto">Each role gets a unique sidebar, color accent, and scope — so staff see only what they need.</p>
          </div>
          <div className="grid md:grid-cols-3 gap-5">
            {[
      ...
      <section id="modules" className="py-20 bg-neutral-900 text-white">
        <DotBackground className="py-4">
          <div className="max-w-6xl mx-auto px-6 text-center">
            <Badge variant="outline" className="border-white/30 text-white/70">Edge Function powered</Badge>
            <h2 className="mt-4 text-4xl font-bold">Smart assignment, realtime by default</h2>
            <p className="mt-3 text-neutral-400 max-w-2xl mx-auto">When a lab result is uploaded, our edge function matches the diagnosed condition to an available specialist and assigns the patient automatically — or broadcasts to all matching doctors if none are free.</p>
          </div>
        </DotBackground>
        <div className="max-w-6xl mx-auto px-6 mt-10 grid md:grid-cols-4 gap-4">
          {[
            ...
            <Card key={s.step} className="p-5 bg-white/5 border-white/10 text-white">
...
export function AuthPage({ portal }: { portal: "patient" | "staff" | "any" }) {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    setInfo("");
    if (mode === "login") {
      // If logging in, send both email and password
      const ok = await login(email, password || "password123");
      if (!ok) {
        setError("Invalid credentials or account is not active yet.");
        return;
      }
      navigate("/dashboard");
    } else {
      if (!fullName || !email) { setError("Please fill all required fields."); return; }
      if (!password) { setError("Password is required to create a secure account."); return; }
  ...
  return (
    <div className="min-h-screen bg-white relative overflow-hidden flex items-center justify-center px-4 py-16">
      ...
      <div className="absolute inset-0 grid-bg opacity-40" />
      <div className="absolute top-0 left-0 right-0 h-[1px] overflow-hidden"><div className="beam-line h-full w-1/3 absolute" /></div>
        ...
        <Card className="p-6">
          ...
          <form onSubmit={onSubmit} className="mt-5 space-y-4">
            ...
            <div>
              ...
              <Input id="pw" type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="••••••••" required={mode === "register"} />
            </div>
